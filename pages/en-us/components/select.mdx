...
<Playground
  title="Use ref."
  desc="Set in focus with the `ref` prop"
  scope={{ Select, Spacer, Button, useRef, useEffect, useState }}
  code={`
() => {
  ...
  const buttonHandler = () => {
    setToFocus(true)
    setTimeout(() => setToFocus(false), 500)
  }
  return (
    <>
      <Select placeholder="Choose one" onChange={handler} ref={selectRef}>
        <Select.Option value="1">Option 1</Select.Option>
        <Select.Option value="2">Option 2</Select.Option>
      </Select>
      <Spacer x={5} inline />
      <Button type="secondary" onClick={buttonHandler}>focus</Button>
    </>
  )
}
`}
...
| **ref**               | forwardRef                                               | <Code>Ref<HTMLDivElement &#124; null></Code>        | -                                 | -               |
